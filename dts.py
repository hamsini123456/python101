#case sensitive
amrita = 1
Amrita  = 1

# variable can start with _ but not a number

price = 20.50
unit = 10

total = price*unit

#order of typecasting  - int < float
# Boolean
amrita = True
Amrita = False

number = 1
print(number)
print(Amrita)


# operation on string
sample = 'good day'
print(sample.upper())
sample = 'GOOD DAY'
print(sample.lower())


# data validation
name = 'aifoss'
number = 123456
symbol = '$'

print(name.isalpha()) # check for alphabets
print(name.isnumeric()) # check for numbers
print(name.isalnum()) # check for alphabets and numbers


print('check symbol')
print(symbol.isalnum())

sample_string = 'a quick brown fox'
# try to findout what the following syntax does
# startswith
# endswith
# find
# index
# startswith
# sample_string = 'a quick brown fox'
# print(sample_string.startswith('a'))  
# print(sample_string.startswith('b')) 
# endswith
# print(sample_string.endswith('x'))
# print(sample_string.endswith('y'))  
# find
# print(sample_string.find('brown'))
# print(sample_string.find('cat'))    
# index
# print(sample_string.index('brown'))



